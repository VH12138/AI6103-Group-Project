...
def text_loss(source, target, args):
    source_feat = source[-1] / source[-1].norm(dim=-1, keepdim=True) #shape = [8, 768]
    target = target / target.norm(dim=-1, keepdim=True) #shape=[8, 512]
    fc = nn.Linear(768, 512, device=target.device)
    source_feat = fc(source_feat)
